...
Page({
  ...
  getDayData(params) {
    ajax('/index/report/cleaningDaily', params).then(res => {
      this.setData({ list: res.cleaningList })
    })
  },
  getMonthData(form) {
    let params;
    ...
    ajax('/index/report/cleaningMonth', params).then(res => {
      const { cleaningList } = res
      const xData = cleaningList.map(item => item.dday.slice(-5))
      const yData = cleaningList.map(item => item.cnt)
